...
def define_data(trial, conn, params, training=True):
    ...
    data.add_features("clnc-1",
        remove=("arrest", "enzymes")
    )

    data.add_features("clnc-2",
        remove=("abnormal-ekg", "abnormal-qrs-st", "dominance",
                "ischemia-test", "nyha")
    )
...
if __name__ == "__main__":
    ...
    # setup logging
    logging.basicConfig(
        filename=snakemake.output["logfile"], filemode="w", level=logging.INFO,
    ...
    with open(snakemake.output["trialobj"], "wb") as f:
        pickle.dump(trial, f)
